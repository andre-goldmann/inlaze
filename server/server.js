...
app.put('/api/user', authenticateToken, async (req, res) => {
  ...
  try {
    await db('users').where({ id: user.userId })
      .update({
        fullName: user.fullName,
        email: user.email,
        age: user.age
...
app.post('/api/login', async (req, res) => {
  ...
  try {
    ...
    if (user.length > 0) {
      const token = jwt.sign({ userId: user.id, username: user.fullName }, process.env.TOKEN_SECRET, { expiresIn: '1h' });
...
app.get('/api/posts', authenticateToken, async (req, res) => {
  ...
  const posts = await db('posts').select('posts.*', 'users.fullName as userName')
...
app.put('/api/post', authenticateToken, async (req, res) => {
  ...
  await db('posts').where({ id: post.postId })
    .update({
...
app.get('/api/posts/search', authenticateToken,async (req, res) => {
  ...
  const posts = await db('posts').select('*', 'users.fullName as userName')
    .leftJoin('users', 'users.id', 'posts.userId')
